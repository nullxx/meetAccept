  ...
  navigator.languages
    ...
    .map(
      (l) =>
        ...
        (translations[l.split("-")[0]] && translations[l.split("-")[0]][key])
    )
    .find((t) => t);
...
<div>
    <label>
        <input type="checkbox" id="meetAcceot_activated">${T(
          "activate"
        )}</label>
</div>
</div>`;
    ...
    document
      .getElementById("meetAcceot_activated")
...
function acceptAll(all) {
  var el = document.querySelectorAll(".g3VIld")[0].children;
  if (el.length > 0) {
    var denegar = el[3].children[0].children[0];
    var aceptar = el[3].children[0].children[1];
    var mensaje = el[1].innerText;
    if (all == false) {
      // TODO customization
    } else {
